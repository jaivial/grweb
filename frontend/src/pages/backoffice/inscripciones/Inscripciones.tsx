...
import { Lock, Unlock, Pencil, Trash2, Check, QrCode, Copy } from 'lucide-react';
import {
...
function OpenerCell({ athleteId, liftType, value, canEdit, onSave }: {
  ...
  if (editing) {
    return (
      <input
        ...
        data-ui={`opener-input-${liftType.toLowerCase()}-${athleteId}`}
        disabled={saving}
      />
    );
  }
  ...
  return (
    <span
      ...
      data-ui={`opener-display-${liftType.toLowerCase()}-${athleteId}`}
    >
      {saving ? (
        <span className="inline-block w-4 h-4 border-2 border-white/20 border-t-white/80 rounded-full animate-spin" data-ui={`opener-spinner-${liftType.toLowerCase()}-${athleteId}`} />
      ) : (
        displayValue
      )}
    </span>
    ...
    <BackofficeLayout>
      ...
      {qrModal && (
        <div
          ...
          data-ui="qr-modal-overlay"
          onClick={() => setQrModal(null)}
        >
          <div
            className="bg-[#1a1a2e] border border-white/10 rounded-2xl p-6 max-w-sm w-full mx-4 shadow-xl"
            data-ui="qr-modal-content"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between mb-4" data-ui="qr-modal-header">
              <h3 className="text-lg font-semibold text-white" data-ui="qr-modal-title">Codigo QR</h3>
              <button
                onClick={() => setQrModal(null)}
                className="p-1 rounded-lg text-white/40 hover:text-white hover:bg-white/10 transition-colors"
                data-ui="qr-modal-close"
                aria-label="Cerrar"
              >
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" data-ui="qr-modal-close-icon">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                </svg>
              </button>
            </div>

            <p className="text-sm text-white/60 mb-2" data-ui="qr-modal-athlete-name">{qrModal.name}</p>

            <div className="bg-white/5 border border-white/10 rounded-xl p-4 mb-4" data-ui="qr-modal-code-box">
              <p className="text-xs text-white/40 mb-1" data-ui="qr-modal-code-label">Codigo</p>
              <p className="font-mono text-base text-white break-all select-all" data-ui="qr-modal-code-value">
                {qrModal.code}
              </p>
            </div>
